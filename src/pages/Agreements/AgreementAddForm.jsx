...
export default function AgreementAddForm({ onSubmit, onCancel }) {
  // --- STATE ---
  const [template, setTemplate] = useState("paddy_receipt");
  ...
  const [formData, setFormData] = useState({
    ...
    // Specifics
    equipmentName: "Rice Polishing Machine",
    ...
    switch (template) {
      ...
      default: 
        ...
        return (
            <>
                <Header />
                <div className="doc-title">{template.replace('_', ' ').toUpperCase()}</div>
                <div style={{marginTop: '30px', minHeight: '400px'}}>
                    ...
                    <div style={{marginTop: '20px', lineHeight: '1.8'}}>
                        <p className="doc-text">{formData.remarks || "Content goes here..."}</p>
                        {formData.amount && <p className="doc-text">Consideration Amount: ₹{formData.amount}</p>}
                    </div>
                </div>
      ...
      <div className="generator-controls">
        ...
        <form onSubmit={handleSubmit} className="control-form">
          ...
          <div className="form-group">
            ...
            <select className="input-field" name="template" value={template} onChange={(e) => setTemplate(e.target.value)}>
          ...
          <div className="form-row">
            <div className="form-group"><label>Cust ID</label><input className="input-field" type="number" name="customerId" value={formData.customerId} onChange={handleChange} /></div>
            <div className="form-group"><label>Date</label><input className="input-field" type="date" name="startDate" value={formData.startDate} onChange={handleChange} /></div>
          </div>
          ...
          {template !== 'notice_template' && (
            <>
                ...
                <div className="form-group"><label>Name / Farmer</label><input className="input-field" type="text" name="party2Name" value={formData.party2Name} onChange={handleChange} /></div>
                <div className="form-group"><label>{template === 'paddy_receipt' ? 'Village' : 'Address'}</label><input className="input-field" type="text" name="party2Address" value={formData.party2Address} onChange={handleChange} /></div>
                
                {template === 'paddy_receipt' && (
                    <div className="form-row">
                        <div className="form-group"><label>Mobile</label><input className="input-field" type="text" name="party2Mobile" value={formData.party2Mobile} onChange={handleChange} /></div>
                        <div className="form-group"><label>Aadhaar</label><input className="input-field" type="text" name="party2Aadhaar" value={formData.party2Aadhaar} onChange={handleChange} /></div>
                    </div>
                )}
            </>
          )}
          ...
          <div className="form-group">
            ...
            <input className="input-field" type="text" name={template === 'paddy_receipt' ? 'vehicleNo' : 'equipmentName'} value={template === 'paddy_receipt' ? formData.vehicleNo : formData.equipmentName} onChange={handleChange} />
          </div>
          
          {template === 'paddy_receipt' && (
            <div className="form-row">
                <div className="form-group"><label>Godown</label><input className="input-field" type="text" name="godown" value={formData.godown} onChange={handleChange} /></div>
                <div className="form-group"><label>Weighbridge Slip</label><input className="input-field" type="text" name="weighbridgeSlip" value={formData.weighbridgeSlip} onChange={handleChange} /></div>
            </div>
          )}
                ...
                {formData.items.map((item, index) => (
                    <div key={index} className="item-card">
                        <div className="form-group">
                          ...
                          <input className="input-field" placeholder={template === 'rtgs_template' ? 'Beneficiary Name' : 'Description/Variety'} value={item.desc} onChange={(e) => handleItemChange(index, 'desc', e.target.value)} />
                        </div>
                        
                        <div className="form-row-tight">
                            {template === 'paddy_receipt' && 
                              <div className="form-group"><label>Lot ID</label><input className="input-field" placeholder="Lot ID" value={item.lot} onChange={(e) => handleItemChange(index, 'lot', e.target.value)} /></div>
                            }
                            {template === 'paddy_receipt' && 
                              <div className="form-group"><label>Vehicle</label><input className="input-field" placeholder="Vehicle" value={item.vehicle} onChange={(e) => handleItemChange(index, 'vehicle', e.target.value)} /></div>
                            }
                        </div>

                        <div className="form-row-tight">
                            <div className="form-group"><label>{template === 'rtgs_template' ? 'A/C No' : 'Qty/Bags'}</label><input className="input-field" type="number" value={item.qty} onChange={(e) => handleItemChange(index, 'qty', e.target.value)} /></div>
                            <div className="form-group"><label>{template === 'rtgs_template' ? 'Amount' : 'Rate'}</label><input className="input-field" type="number" value={item.rate} onChange={(e) => handleItemChange(index, 'rate', e.target.value)} /></div>
                        </div>
                        
                        {(template === 'paddy_receipt' || template === 'rtgs_template') && (
                            <div className="form-group">
                              <label>{template === 'paddy_receipt' ? "Gross Weight (Kg)" : "IFSC Code"}</label>
                              <input className="input-field" value={item.unit} onChange={(e) => handleItemChange(index, 'unit', e.target.value)} />
                            </div>
                        )}
                        <button type="button" className="btn-danger-sm" onClick={() => removeItem(index)}>Remove Row</button>
                    </div>
                ))}
                <button type="button" className="btn-add-row" onClick={addItem}>+ Add Row</button>
            </div>
          )}
          ...
          {template === 'paddy_receipt' && (
            <>
                ...
                <div className="form-row">
                    <div className="form-group"><label>Moisture</label><input className="input-field" type="number" name="deductionMoisture" value={formData.deductionMoisture} onChange={handleChange} /></div>
                    <div className="form-group"><label>Transport</label><input className="input-field" type="number" name="deductionTransport" value={formData.deductionTransport} onChange={handleChange} /></div>
                </div>
                <div className="form-row">
                    <div className="form-group"><label>Packaging</label><input className="input-field" type="number" name="deductionPackaging" value={formData.deductionPackaging} onChange={handleChange} /></div>
                    <div className="form-group"><label>Other</label><input className="input-field" type="number" name="deductionOther" value={formData.deductionOther} onChange={handleChange} /></div>
                </div>
            </>
          )}
          ...
          {/* 7. General */}
          {!['professional_invoice', 'paddy_receipt', 'rtgs_template'].includes(template) && (
             <div className="form-group"><label>Amount</label><input className="input-field" type="number" name="amount" value={formData.amount} onChange={handleChange} /></div>
          )}
          
          <div className="form-group"><label>Remarks</label><textarea className="input-field" name="remarks" rows={3} value={formData.remarks} onChange={handleChange} /></div>
